...
void main() {
  ...
  testWidgets('typing a qty and pressing next advances the deck focused',
      (tester) async {
    ...
    expect(
        tester.widget<TextField>(topField).focusNode?.hasFocus, isTrue,
    ...
    final newTop = find.byWidgetPredicate(
        (w) => w is TextField && w.focusNode != null);
    expect(tester.widget<TextField>(newTop).focusNode?.hasFocus, isTrue,
  ...
  testWidgets('hardware Enter key advances the deck (desktop path)',
      (tester) async {
    final set = <String, double>{};
    await tester.pumpWidget(MaterialApp(
      localizationsDelegates: AppLocalizations.localizationsDelegates,
